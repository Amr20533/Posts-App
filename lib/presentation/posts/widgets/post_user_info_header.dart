...
class PostUserInfoHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(create: (context) => ProfileCubit()..getUserInfo(post.user.toString()),
                  ...
                  children: [
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        ...
                        UbuntuText(text: '${userInfo.firstName ?? 'User'} ${userInfo.lastName ?? 'Name'}', height: 1.3,color: AppColors.black,fontSize: 20, fontWeight: FontWeight.w500,maxLines: 1),

                        Icon(Icons.more_horiz_sharp, color: AppColors.black, size: 24,)
                      ],
                    ),
                    UbuntuText(text: FormatHelper.getFormattedDate(post.createdAt ?? DateTime.now()), height: 1.3, )
                  ],
                )),
              ],
            );
